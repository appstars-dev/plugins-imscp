...
my @BUILD_DEPS = (
    ...
    'libmcrypt-dev', 'libmhash-dev', 'libmysqlclient-dev', 'libncurses5-dev', 'libonig-dev', 'libpam0g-dev', 'libpcre3-dev',
    'libpng-dev', 'libpng12-dev', 'libpq-dev', 'libpspell-dev', 'libqdbm-dev', 'librecode-dev', 'libsasl2-dev', 'libsnmp-dev',
    'libsqlite3-dev', 'libssl-dev', 'libtidy-dev', 'libtool', 'libvpx-dev', 'libwrap0-dev', 'libxml2-dev', 'libxmltok1-dev',
    'libxslt1-dev', 'locales-all', 'netbase', 'netcat-traditional', 'quilt', 're2c', 'systemtap-sdt-dev', 'unixodbc-dev',
    'zlib1g-dev', 'build-essential', 'shtool', 'wget', 'libsqlite0-dev', 'lemon', 'snmp'
);
...
This script allows to download, configure, compile and install one or many PHP versions in one step.
...
OPTIONS:
 -b,    --build-dir <DIR>   Build directory ( /usr/local/src/phpswitcher ).
 -i,    --install-dir <DIR> Base installation directory ( /opt/phpswitcher ).
 -d,    --download-only     Download only.
 -f,    --force-last        Force use of the last available PHP versions.
 -j,    --parallel-jobs <N> Number of parallel jobs for make ( default: 2 ).
 -d,    --dev-mode <MODE>   Development mode ( patch, configure or build )
 -v,    --verbose           Enable verbose mode.},
 'build-dir|b=s' => sub { setOptions(@_); },
 'install-dir|i=s' => sub { setOptions(@_); },
...
for my $sVersion(@sVersions) {
    ...
    unless($DOWNLOAD_ONLY) {
        ...
        process($sVersion, $lVersion) unless $DEV_MODE eq 'patch';
...
sub setOptions
{
    ...
    if($option eq 'build-dir') {
        $BUILD_DIR = $value;
        print output(sprintf('Build directory set to %s', $value), 'info');
    } elsif($option eq 'install-dir') {
        if(-d $value) {
            $INSTALL_DIR = $value;
            print output(sprintf('Base installation directory set to %s', $value), 'info');
        } else {
            die("Directory speficied by the --install-dir option must exists.\n");
        }
    } elsif($option eq 'parallel-jobs') {
        $PARALLEL_JOBS = $value;
        print output(sprintf('Parallel jobs set to %s', $value), 'info');
    } elsif($option eq 'dev-mode') {
        if($value ~~ [ 'patch', 'configure', 'build' ]) {
            $DEV_MODE = $value;
            print output(sprintf('Development mode set to %s', $value), 'info');
        } else {
...
sub installBuildDeps
{
    ...
    # Filter packages which are not available since the build dependencies list is a mix of packages that were pulled
    # from different Debian php5 package control files
    my ($stdout, $stderr);
...
sub getLongVersion
{
    ...
    my ($ret, $foundUrl) = (0, undef);

    # Scan the PHP museum. This covers the versions which are end of life and the versions which were moved since the
    # last release of this script.
    do {
    ...
    # Scan PHP mirrors only if needed
...
sub process
{
    ...
    my $stderr;
    (
        execute("make -f $Bin/Makefile PREFIX=$installDir TPLS_DESTDIR=$Bin/templates $target", undef, \$stderr) == 0
    ) or fatal(sprintf(
